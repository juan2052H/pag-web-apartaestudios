...
const { generarCodigo, verificarCodigo } = require('./recuperacion');
...
/**
 * Genera un código de recuperación para `clave` y lo envía por correo. Si no
 * hay Resend configurado (o el envío falla por cualquier razón), lo deja
 * impreso en consola como respaldo — así el flujo sigue siendo probable en
 * desarrollo local y en la suite de pruebas sin depender de un servicio real.
 */
async function enviarCodigoRecuperacion({ clave, correo, asunto, cuerpoHtml }) {
  const codigo = generarCodigo(clave);
  const { enviado, motivo } = await enviarCorreo({ to: correo, asunto, html: cuerpoHtml(codigo) });
  if (!enviado) {
    console.log(`  [recuperación] Código para "${clave}": ${codigo}${motivo ? ` (correo no enviado: ${motivo})` : ''}`);
  }
}
...
async function manejarApi(req, res, url) {
  ...
  if (seccion === 'inquilino') {
    ...
    if (recurso === 'recuperar' && m === 'POST') {
      const b = await leerJson(req);
      const documento = documentoNormalizado(b.documento);
      if (limiteExcedido(`recuperar-inquilino|${documento}|${texto(req.socket?.remoteAddress, 80)}`, 3, 15 * 60 * 1000)) {
        return error(res, 429, 'Demasiadas solicitudes. Intenta de nuevo más tarde.');
      }
      const c = db.contratos.find((x) =>
        x.estado === 'activo' && x.portal?.activo && documento &&
        documentoNormalizado(x.inquilino?.documento) === documento);
      if (c && c.inquilino?.email) {
        await enviarCodigoRecuperacion({
          clave: `inquilino:${c.id}`,
          correo: c.inquilino.email,
          asunto: 'Código para restablecer tu contraseña del portal',
          cuerpoHtml: (codigo) => `<p>Tu código para restablecer la contraseña del portal es:</p><p style="font-size:24px;font-weight:bold">${codigo}</p><p>Vence en 15 minutos. Si no lo pediste tú, ignora este correo.</p>`,
        });
      }
      return ok(res, { ok: true, mensaje: 'Si el documento existe y tiene un correo registrado, te enviamos un código.' });
    }

    if (recurso === 'recuperar-confirmar' && m === 'POST') {
      const b = await leerJson(req);
      const documento = documentoNormalizado(b.documento);
      const c = db.contratos.find((x) =>
        x.estado === 'activo' && x.portal?.activo && documento &&
        documentoNormalizado(x.inquilino?.documento) === documento);
      if (!c || !verificarCodigo(`inquilino:${c.id}`, texto(b.codigo, 6))) {
        return error(res, 400, 'Código incorrecto o expirado.');
      }
      if (String(b.nueva || '').length < 8) return error(res, 400, 'La nueva contraseña debe tener al menos 8 caracteres.');
      const credencial = hashClave(b.nueva);
      c.portal.sal = credencial.sal;
      c.portal.hash = credencial.hash;
      c.portal.actualizado = ahora();
      invalidarSesionInquilinoDeContrato(c.id);
      registrarAuditoria(null, 'Contraseña del portal restablecida por recuperación', `Contrato ${c.id.slice(0, 6)}.`);
      await guardarDb();
      return ok(res);
    }
  ...
  if (seccion === 'auth') {
    ...
    if (recurso === 'recuperar' && m === 'POST') {
      const b = await leerJson(req);
      const usuario = texto(b.usuario, 40).trim().toLowerCase();
      if (limiteExcedido(`recuperar-admin|${usuario}|${texto(req.socket?.remoteAddress, 80)}`, 3, 15 * 60 * 1000)) {
        return error(res, 429, 'Demasiadas solicitudes. Intenta de nuevo más tarde.');
      }
      const admin = db.administradores.find((x) => x.activo && String(x.usuario).toLowerCase() === usuario);
      if (admin && admin.email) {
        await enviarCodigoRecuperacion({
          clave: `admin:${admin.id}`,
          correo: admin.email,
          asunto: 'Código para restablecer tu contraseña',
          cuerpoHtml: (codigo) => `<p>Tu código para restablecer la contraseña del panel es:</p><p style="font-size:24px;font-weight:bold">${codigo}</p><p>Vence en 15 minutos. Si no lo pediste tú, ignora este correo.</p>`,
        });
      }
      return ok(res, { ok: true, mensaje: 'Si el usuario existe y tiene un correo registrado, te enviamos un código.' });
    }
    ...
    if (recurso === 'recuperar-confirmar' && m === 'POST') {
      ...
      const usuario = texto(b.usuario, 40).trim().toLowerCase();
      const admin = db.administradores.find((x) => x.activo && String(x.usuario).toLowerCase() === usuario);
      if (!admin || !verificarCodigo(`admin:${admin.id}`, texto(b.codigo, 6))) {
        return error(res, 400, 'Código incorrecto o expirado.');
      }
      if (String(b.nueva || '').length < 8) return error(res, 400, 'La nueva contraseña debe tener al menos 8 caracteres.');
      ...
      registrarAuditoria(null, 'Contraseña restablecida por recuperación', `Cuenta: ${admin.usuario}.`);
      await guardarDb();
      return ok(res);
    }
    ...
    if (recurso === 'clave' && m === 'POST') {
      ...
      const email = texto(b.email, 160).trim();
      if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
        return error(res, 400, 'El correo no parece válido.');
      }
      const nuevo = hashClave(b.nueva);
      admin.sal = nuevo.sal;
      admin.hash = nuevo.hash;
  ...
  if (seccion === 'administradores') {
    ...
    if (m === 'POST' && !recurso) {
      ...
      const nuevo = {
        id: id(), nombre, usuario, email, rol: 'edificio', edificioIds,
